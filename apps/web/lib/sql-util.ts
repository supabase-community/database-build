...
/**
 * Filters strings ending in the specified suffix.
 */
export type StringWithSuffix<T extends string, S extends string> = T extends `${infer _Prefix}${S}`
  ? T
  : never

/**
 * Extracts all nodes who's keys end in the specified suffix.
 */
export type NodeWithSuffix<S extends string> = Node extends infer U
  ? U extends {
      [K in keyof U]: K extends `${infer _Prefix}${S}` ? U : never
    }[keyof U]
    ? U[keyof U]
    : never
  : never
 ...
 * Every Node type.
 */
export type NodeType = ExtractKeys<Node>

/**
 * Every statement Node type.
 */
export type StatementType = StringWithSuffix<NodeType, 'Stmt'>

/**
 * Every statement Node.
 */
export type Statement = NodeWithSuffix<'Stmt'>

/**
 * Unwraps a Node to get its underlying value.
 */
export type NodeValue<T extends Node, U extends NodeType> = T extends Record<U, infer V> ? V : never
 ...
 */
export function assertAndUnwrapNode<T extends Node, U extends NodeType>(
  node: Node,
  type: U,
  errorMessage: string
): NodeValue<T, U> {
 ...
 */
export function unwrapNode<T extends Node, U extends NodeType>(
  node: Node,
  type: U
 ...
 *
 * @returns The unwrapped `Node` value or `undefined` if
 * the node is not one of the node types in `types`.
 */
export function unwrapOneOfNode<T extends Node, U extends readonly [NodeType, ...NodeType[]]>(
  node: Node,
  types: U
): { [K in keyof U]: NodeValue<T, U[K]> }[number] | undefined {
  const nodeType = types.find((type) => unwrapNode(node, type))

  if (nodeType) {
    return unwrapNode(node, nodeType)
  }
}
